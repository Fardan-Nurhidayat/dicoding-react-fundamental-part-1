...
import { getNoteById, archiveNote, unarchiveNote, deleteNote } from "@utils/api";
...
export default function DetailPage() {
    ...
    (async () => {
      try {
        if (note.archived) {
          ...
          setNote((prev) => ({ ...prev, archived: true }));
        }
      } catch (err) {
        console.error(err);
      }
  ...
  const handleDelete = () => {
    if (window.confirm(t("detailPage.confirmDelete"))) {
      (async () => {
        try {
          await deleteNote(id);
        } catch (err) {
    ...
    return (
      <div className="col-span-10 flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-2">{t("detailPage.noteNotFoundTitle")}</h2>
          <p className="text-gray-600 dark:text-gray-400 mb-6">{t("detailPage.noteNotFoundMessage")}</p>
          <button
            onClick={handleBack}
    ...
    <div className="col-span-10 bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-300">
              ...
              >
                {note.archived ? (
                  <>
                    ...
                    {t("detailPage.unarchive")}
                  </>
                ) : (
                  <>
                    <ArchiveBoxIcon className="w-5 h-5" />
                    {t("detailPage.archive")}
                  </>
                )}
              </button>
              <button
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
            ...
            <div className="mb-6">
              <div className="flex items-center gap-3 mb-4">
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                  ...
                  {note.archived ? t("detailPage.archived") : t("detailPage.active")}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-400">
                <div className="flex items-center gap-2">
                  ...
                  <span>{t("detailPage.created")}: {formatedDate(note.createdAt)}</span>
                </div>
                {note.updatedAt && note.updatedAt !== note.createdAt && (
                  <div className="flex items-center gap-2">
                    <ClockIcon className="w-5 h-5 text-gray-400 dark:text-gray-500" />
                    <span>{t("detailPage.updated")}: {formatedDate(note.updatedAt)}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="mt-6 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-lg p-4 transition-colors duration-300">
            ...
            <span className="font-semibold">{t("detailPage.tipTitle")}</span> {t("detailPage.tipMessage")}
          </p>
        </div>
      </div>
    </div>
